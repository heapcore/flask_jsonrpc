    ...
    def process_request(self, items):
        ...
        try:
            if not isinstance(items, dict):
                raise SimpleJsonRpcError(message='JSON-RPC: Invalid request format', code=-32600)
            ...
            if hasattr(handler, 'nolog'):
                logger.debug('JSON-RPC: {} ({:.2f})'.format(handler_name, (time_end - time_start)))
            else:
                logger.debug('JSON-RPC: {} {} ({:.2f})'.format(handler_name, kwargs, (time_end - time_start)))
        except SimpleJsonRpcError as e:
            error = e
        except Exception:
            message = traceback.format_exc()
            error = SimpleJsonRpcError(message=message, code=-32603)
            logger.error('JSON-RPC: Failed to process request {}: {}'.format(handler_name, message))
