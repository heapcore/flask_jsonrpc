...
class SimpleJsonRpcError(Exception):
    ...
    def __str__(self):
        return "<JsonRpcError with message {} and code {}>".format(
...
class SimpleJsonRpcServer(object):
    ...
    def prepare_request(self):
        ...
        if "application/json" not in content_type or not request.data:
            logger.error("Invalid Content-Type: " + content_type)
            abort(404)
    ...
    def process_request(self, items):
        protocol, tid, action, handler_name, result, error = (
            None,
            None,
            None,
            None,
        ...
        try:
            if not isinstance(items, dict):
                raise SimpleJsonRpcError(
                    message="JSON-RPC: Invalid request format", code=-32600
                )

            args, kwargs = self.__expand_params(items.get("params"))
            handler_name = items.get("method")
            protocol = items.get("version", "2.0")
            tid = items.get("id")

            if not handler_name or not protocol or ("id" not in items):
            ...
            if hasattr(handler, "nolog"):
                logger.debug("JSON-RPC: " + handler_name)
            else:
                logger.debug("JSON-RPC: {} {}".format(handler_name, kwargs))
            ...
            if hasattr(handler, "nolog"):
                logger.debug(
                    "JSON-RPC: {} ({:.2f})".format(
                        handler_name, (time_end - time_start)
                    )
                )
            else:
                logger.debug(
                    "JSON-RPC: {} {} ({:.2f})".format(
                        handler_name, kwargs, (time_end - time_start)
                    )
                )
            ...
            logger.error(
                "JSON-RPC: Failed to process request {}: {}".format(
...
class SimpleJsonRpcClient(object):
    ...
    def __repr__(self):
        return json.dumps({"version": self.version, "method": self.service_name})
